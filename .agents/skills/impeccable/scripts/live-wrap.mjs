...
function applyBufferedManualEditToLines(originalLines, selectionStartLine, op) {
  if (
    ...
    return { changed: false, lines: originalLines };
  }
...
function findElement(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    return { endLine, startLine: openerLine };
  }
...
function findAllElements(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    seen.add(openerLine);
    const endLine = findClosingLine(lines, openerLine);
    out.push({ endLine, startLine: openerLine });
  }
  return out;
}
...
function filterByText(candidates, lines, text) {
  ...
  const targetCompact = trimmed.replaceAll(/\s+/g, "");

  return candidates.filter((c) => {
    const body = lines.slice(c.startLine, c.endLine + 1).join(" ");
    const inner = body
      .replaceAll(/<[^>]*>/g, " ") // strip HTML/JSX tags
      .replaceAll(/\{[^}]*\}/g, " ") // strip JSX expressions
      .toLowerCase();
    const sourceSpaced = inner.replaceAll(/\s+/g, " ").trim();
    const sourceCompact = inner.replaceAll(/\s+/g, "");
    return (
