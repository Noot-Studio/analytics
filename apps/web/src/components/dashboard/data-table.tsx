...
export type DataRow = z.infer<typeof schema>;
...
const TableCellViewer = ({ item }: { item: DataRow }) => {
        ...
        <div className="flex flex-col gap-4 overflow-y-auto px-4 text-sm">
          ...
          <div className="grid gap-2">
            ...
            <div className="text-muted-foreground">
              Showing total visitors for the last 6 months. This data is updated
              daily.
            </div>
          </div>
          <Separator />
          ...
          <div className="grid gap-3">
            ...
            <div className="grid gap-2">
              ...
              <Select defaultValue={item.reviewer}>
                <SelectTrigger id={`reviewer-${item.id}`} className="w-full">
                  <SelectValue placeholder="Select a reviewer" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="Eddie Lake">Eddie Lake</SelectItem>
                  <SelectItem value="Olivia Martin">Olivia Martin</SelectItem>
                  <SelectItem value="Sophia Chen">Sophia Chen</SelectItem>
                  <SelectItem value="Marcus Reed">Marcus Reed</SelectItem>
...
export const DataTable = ({ data }: { data: DataRow[] }) => {
  const [sort] = useQueryState(
    "sort",
    getSortingStateParser<DataRow>().withDefault([])
  );
  ...
  const sortedData = React.useMemo(() => {
    ...
    return [...data].toSorted((a, b) => {
      const av = a[id as keyof DataRow];
      const bv = b[id as keyof DataRow];
  ...
  const pagedData = sortedData.slice(start, start + perPage);
...
export const DataTablePage = () => {
  const data = React.useMemo(
    () => dataJson.map((row) => schema.parse(row)),
    []
  );
